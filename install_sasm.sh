...
proot-distro login ubuntu --user $varname --shared-tmp -- env DISPLAY=:1.0 sudo apt install sasm
proot-distro login ubuntu --user $varname --shared-tmp -- env DISPLAY=:1.0 echo "alias sasm='QT_SCALE_FACTOR=2 sasm'" >> $PREFIX/var/lib/proot-distro/installed-rootfs/ubuntu/home/$varname/.bashrc
...
GenericName=sasm
Comment=sasm
Exec=proot-distro login ubuntu --user $varname --shared-tmp -- env DISPLAY=:1.0 QT_SCALE_FACTOR=2 sasm
